...
For the current three-protein project, four open actions were explicitly added
after reviewing the supplied archive:
...
The fifth open action, `select-protein-expression-host`, is generated by a coded
rule. In particular, the active stage-1 FASTA contains only the three original
CDS records. The current program would not rediscover that the separately
supplied "B5 optimized CDS" translates to A33 if that explicit action were
removed from the project configuration.
