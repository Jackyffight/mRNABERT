...
ATTENTION_BACKENDS = {"pytorch", "remote-safe", "remote-triton"}
INIT_MODES = {"scratch", "pretrained"}
...
class ModelRuntimeConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.init_mode not in INIT_MODES:
            allowed = ", ".join(sorted(INIT_MODES))
...
def _force_pytorch_attention_if_needed(config: object, runtime: ModelRuntimeConfig) -> None:
    if runtime.init_mode != "pretrained":
        return
...
def load_mlm_model_and_tokenizer(runtime: ModelRuntimeConfig) -> ModelBundle:
    ...
    The default path is ``scratch``: load a local BERT config/tokenizer and
    initialize weights randomly. ``pretrained`` remains available for explicit
    baseline runs against YYLY66/mRNABERT.
    """
    ...
    tokenizer_trust_remote_code = runtime.init_mode == "pretrained"
    ...
    if runtime.init_mode == "scratch":
        logger.info("Initializing masked-LM model from scratch with config: %s", config_source)
        model = AutoModelForMaskedLM.from_config(config)
    else:
        logger.info("Loading pretrained masked-LM weights from: %s", runtime.model_name_or_path)
